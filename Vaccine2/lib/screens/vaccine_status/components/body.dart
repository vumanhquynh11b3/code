    ...
    Future.delayed(Duration.zero, () async {
      ...
      SqlConn.readData(
              "select [dbo].[DSDangKyTiem].TrangThai,[dbo].[DotTiemChung].LoaiVaccine,[dbo].[DotTiemChung].NgayTiem from [dbo].[DSDangKyTiem] INNER JOIN [dbo].[DotTiemChung] on [dbo].[DSDangKyTiem].ID_DotTiem=[dbo].[DotTiemChung].ID_DotTiem "
                      "Where [dbo].[DSDangKyTiem].ID_CongDan='" +
                  check_id_congdan +
                  "'")
          .then((value) {
        setState(() {
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(
                  'Ngày tiêm: ${actor.dt.day}-${actor.dt.month}-${actor.dt.year} ',
                  style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                  ),
                ...
                if (actor.tt == 0)
                  ...
                  Text('Trạng Thái: Sức khỏe không đạt',
                      style: const TextStyle(
                        color: Colors.black,
                        fontWeight: FontWeight.bold,
                      ))
                ...
                if (actor.dt.toString().compareTo(DateTime.now().toString()) >
                    0)
                  Text(
                    'Chưa đến ngày tiêm',
                    style: const TextStyle(
                      ...
                      fontWeight: FontWeight.bold,
                        ...
                        .format(actor.dt)
                        .compareTo(DateFormat.yMMMd().format(DateTime.now())) ==
                    0)
                  Text(
                    'Đã đến ngày tiêm',
                    style: const TextStyle(
